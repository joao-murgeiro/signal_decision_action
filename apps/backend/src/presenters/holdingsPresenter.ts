...
import { assertSymbolAllowed, lookupSymbolLabel } from "../services/etfSymbolService.js";

// Map holding rows to API response shape.
...
// Validate input, enforce symbol rules, and create or increment a holding.
export async function createHolding(
  ...
  const label = trimmedLabel && trimmedLabel.length > 0 ? trimmedLabel : await lookupSymbolLabel(input.symbol);
  try {
    const id = holdingsModel.createHolding(db, { ...input, label });
    ...
    if (isUniqueConflict) {
      const changes = holdingsModel.incrementHolding(db, input.symbol, input.shares, input.targetWeight, label ?? null);
      if (changes > 0) {
...
// Validate input and update a holding row.
